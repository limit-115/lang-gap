...
Llang Gap — открытая платформа для изучения различий в качестве LLM между языками и способов уменьшить эти различия.
...
Для современных reasoning API потребуется явно описанная адаптация: официальные шаблоны содержат `temperature: 0` и ограничение генерации 2048 токена, которые нельзя механически переносить на все модели. Поэтому название нашего протокола — например, `mmluprox-lite-5shot-native-reasoning-v1`; он не объявляется точным воспроизведением результатов статьи. Все отличия в формате API, лимитах и извлечении ответа перечисляются в методике. [Исходный Lite-шаблон](https://github.com/EleutherAI/lm-evaluation-harness/blob/main/lm_eval/tasks/mmlu_prox/en/_en_lite_template_yaml), [ограничения GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model).
...
- Лимиты вывода подбираются в техническом пилоте и затем фиксируются. Для пары EN/RU одной конфигурации лимит одинаковый; языки могут фактически расходовать разное число токенов.
...
| Ответ обрезан лимитом                            | Фиксируем 0 и truncation; выпуск не проходит проверку полноты ответа |

При truncation пересматриваем лимит как новый протокол/конфигурацию и заново запускаем затронутую пару EN/RU целиком. Выборочно улучшать только неудачные ответы нельзя. Парный gap публикуется при одинаковом полном наборе заданий и повторов на обоих языках.
