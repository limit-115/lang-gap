export default {
  ...
  protocol:
    "Each request contains the localized instruction, five worked validation examples from the same subject, and one test question. English and Russian use the same question IDs, answer order and settings. There is no conversation history, browsing or tool use.",
  adaptation:
  ...
  score:
    "Accuracy is the proportion of correct answers, with equal weight per question. We average the prespecified repeats; we do not select the best answer. Gap = 100 × (English accuracy − Russian accuracy), in percentage points.",
  interval:
    "The 95% percentile interval uses 10,000 seeded paired bootstrap samples over unique question IDs, preserving both languages and all repeats together. Variation between repeats is reported separately. Effort labels do not imply equal compute across providers.",
